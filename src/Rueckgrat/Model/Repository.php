...
abstract class Repository
{
    ...
    public function save(Entity $entity) {
        ...
        foreach ($entity->getValues() as $column => $value) {
            ...
            $columnName = ':' . $column;
            $queryValues[$columnName] = utf8_decode($value);
            $values[$column] = utf8_decode($value);
        }
...
//        }
    ...
    protected function getTableName(Repository $repository)
    {
        ...
        $tableNamePartsString = preg_replace('/([a-z0-9])?([A-Z])/', '$1 $2', lcfirst($fullRepositoryName));
        $tableNameParts = explode(' ', $tableNamePartsString);
        $tableName = $tableNameParts[0];
        foreach ($tableNameParts as $key => $tableNamePart) {
            if ($key != 0) {
                $tableName .= '_' . lcfirst($tableNamePart);
            }
        }
        return $tableName;
    }
